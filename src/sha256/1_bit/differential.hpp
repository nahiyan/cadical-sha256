...
namespace SHA256 {
inline void get_1bit_differential (OperationId op_id, int step_i,
                                   int bit_pos, State &state,
  ...
  for (int x = 0; x < input_size; x++) {
    ...
    diff.ids.first.push_back (
        {input_word.ids_f[j], input_word.ids_g[j], input_word.char_ids[j]});

    diff.table_values.first.push_back (
        gc_values_1bit (*input_word.chars[j]));
  }
  assert (int (diff.inputs.size ()) == input_size);
  assert (int (diff.ids.first.size ()) == input_size);
  assert (int (diff.table_values.first.size ()) == input_size);

  for (int x = 0; x < output_size; x++) {
    auto &output_word = output_words[x];
    diff.outputs += output_word->chars[j];
    diff.ids.second.push_back ({output_word->ids_f[j],
                                output_word->ids_g[j],
                                output_word->char_ids[j]});
    diff.table_values.second.push_back (
        gc_values_1bit (output_word->chars[j]));
  }
  assert (int (diff.outputs.size ()) == output_size);
  assert (int (diff.ids.second.size ()) == output_size);
